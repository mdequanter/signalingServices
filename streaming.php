...
<head>
  ...
  <title>Webcam â†’ WebSocket Sender</title>
  <style>
...
(() => {
  ...
  async function start() {
    ...
    try {
      ...
      // optional: if server sends messages
      ws.onmessage = (msg) => {
        let heading = null;
        let frameId = null;
  ...
  function updateRate(bytesJustSent) {
    ...
    if (dt >= 1000) {
      const kbps = (bytesSince * 8) / dt; // kilobits per ms => Mbps? wait:
      // bytes*8 bits over dt(ms) => kbits/s = (bytes*8)/dt
